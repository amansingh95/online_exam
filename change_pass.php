...
body>#login #dont_have_an_account a:hover { border-bottom: 2px solid #f90 }
  ...
  <form id="login_form" method="post">
    <div class="field_container">
      <input name="change_pass" type="text" id="change_pass" placeholder="Enter Old password">
      <input name="email" type="text" id="email" placeholder="Enter Email">
    </div>
    
    <div class="field_container">
      <input name="change_button"  type="submit" id="change_button" value="Submit" >
    </div>
    ...
    <div id="sign_in_options" class="field_container">
      <div class="clearfix"></div>
						...
						if(isset($_REQUEST['change_button']))
						{
							 ...
							 $pass_email=$_REQUEST['email'];
                             $sql="select * from myproject.stud_reg";
                             $result=mysql_query($sql);
                               while($row=mysql_fetch_row($result))
	                             { 
								   if($row[6]==$change_pass && $row[5]==$pass_email )
								   {   
								   session_start();
